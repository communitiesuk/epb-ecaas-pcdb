...
pub async fn transform(
    heat_pump_hw_only: &mut Map<std::string::String, JsonValue>,
    products: &HashMap<String, Product>,
    in_use_factors_access: &impl InUseFactorsAccess,
    energy_supplies: &EnergySupplies,
) -> ResolveProductsResult<()> {
    if !heat_pump_hw_only.contains_key(PRODUCT_REFERENCE_FIELD) {
        return Ok(());
    }

    let product_reference = product_reference_from_json_object(heat_pump_hw_only)?;
    ...
    } = &product.technology
    {
        ...
        heat_pump_hw_only.insert("EnergySupply".into(), json!(energy_supply.as_ref()));
        heat_pump_hw_only.insert("power_max".into(), power_max.as_f64().into());
        heat_pump_hw_only.insert(
        ...
        if let Some(heat_exchanger_surface_area_declared) = heat_exchanger_surface_area_declared {
            heat_pump_hw_only.insert(
        ...
        let in_use_factor_mismatch = hot_water_in_use_factors
            ...
            .find(|entry| {
                Option::<HeatPumpVesselType>::try_from(*entry)
                    .ok()
                    .flatten()
                    .is_some_and(|entry_vessel_type| entry_vessel_type == *vessel_type)
            })
        ...
        heat_pump_hw_only.remove(PRODUCT_REFERENCE_FIELD);
    } else {
